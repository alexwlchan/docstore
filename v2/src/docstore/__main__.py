...
def add(root, path, title, tags, source_url, date_created):
    ...
    store_new_document(
        root=root, path=path, title=title, tags=tags, source_url=source_url, date_saved=date_saved
    )


@main.command(help="Migrate a V1 docstore")
@click.option('--root', default='.', help='The root of the docstore database.', type=click.Path(), show_default=True)
@click.option("--v1_path", help='Path to the root of the V1 instance.', type=click.Path(), required=True)
